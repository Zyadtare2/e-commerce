...
import { getCategories, getSubcategories, getBrands } from "../services/sidebarService";
...
const Sidebar: React.FC<SidebarProps> = ({ showMobileSidebar, closeSidebar }) => {
  const [categories, setCategories] = useState<Category[]>([]);
  const [subcategories, setSubcategories] = useState<Subcategory[]>([]);
  const [brands, setBrands] = useState<Brand[]>([]);
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [selectedSubcategory, setSelectedSubcategory] = useState<string | null>(null);
  const [expandedSection, setExpandedSection] = useState<"categories" | "brands" | null>("categories");
  ...
  const handleSubcategoryClick = async (subcategoryId: string) => {
    if (selectedSubcategory === subcategoryId) {
      ...
      try {
        const fetchedBrands = await getBrands(subcategoryId); // ✅ Fetch brands by subcategory
        ...
        <div className="border-b border-gray-100 pb-4">
          ...
          >
            ...
            <svg
              ...
              className={`h-5 w-5 text-gray-500 transform transition-transform ${expandedSection === "categories" ? "rotate-180" : ""}`}
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
              ...
              {categories.map((category) => (
                <li key={category.id} className="group">
                  ...
                  >
                    ...
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className={`h-4 w-4 transform transition-transform ${selectedCategory === category.id ? "rotate-90" : ""}`}
                      viewBox="0 0 20 20"
                      fill="currentColor"
                    >
                      <path
